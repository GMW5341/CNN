...
export async function runInference(
  ...
  let multiModel: tf.LayersModel | null = null;
  let outputs: tf.Tensor[] = [];

  try {
    multiModel = buildMultiOutputModel(model);
    const prediction = multiModel.predict(inputTensor);
    outputs = Array.isArray(prediction) ? prediction : [prediction];
    ...
    // Extract final scores (dense_out) - sigmoid output scaled to 0-100
    ...
    const layerWeights = denseLayer.getWeights();
    const weightsT = layerWeights[0];
    const biasesT = layerWeights[1];
    ...
    if (multiModel) multiModel.dispose();
  }
}
